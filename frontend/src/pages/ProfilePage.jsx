                            ...
                            <div className="card-body p-8">
                                ...
                                {(userData.resume || isEditing) && (
                                    <div className="alert alert-info">
                                        <FileText size={24} />
                                        <div className="flex-1">
                                            <h3 className="font-bold">Resume {userData.resume ? 'on File' : ''}</h3>
                                            <div className="text-xs">{userData.resume ? 'Your resume is stored and ready to be viewed or downloaded' : 'Upload your resume'}</div>
                                        </div>
                                        {isEditing ? (
                                            <div className="flex gap-2 items-center">
                                                <input type="file" className="file-input file-input-bordered file-input-xs" accept=".pdf,.doc,.docx" onChange={handleResumeChange} />
                                                {selectedResume && (
                                                    <button onClick={handleResumeUpload} className="btn btn-success btn-xs">
                                                        <Upload size={14} />
                                                        Upload
                                                    </button>
                                                )}
                                                {userData.resume && (
                                                    <button onClick={handleResumeDelete} className="btn btn-error btn-xs">
                                                        <Trash2 size={14} />
                                                    </button>
                                                )}
                                            </div>
                                        ) : (
                                            userData.resume && <button onClick={handleViewResume} className="btn btn-sm btn-primary">View</button>
                                        )}
                                    </div>
                                )}
                            </div>
